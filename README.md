...
- **Step 2: Server sends request to DB to check Refresh Token from body**<br>
    Refresh Token must exist in DB, must not be used, expired or revoked as well as it's AccessTokenId property  has to be the same as  AccessToken Id from body's Access Token.<br><br>
...
<td  colspan="2">
 ...
 - **Step 1: User send request to invalidate all tokens of a specific user** <br>
    Server checks user's HTTP Authorization header for JWT bearer token and user's Claims in the Access Token.
    If that token and claims are valid check if specific user exit in DB and it that is true go to step 2 and 3.<br><br>
- **Step 2: Server sends request to Redis to expire/delete all Access Tokens of a specific user**<br><br>
- **Step 3: Server sends request to DB to revoke all unrevoked and unexpired Refresh Tokens of a specific user**<br><br>       
